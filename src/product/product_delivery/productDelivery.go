...
func (c *productDelivery) GetAllProducts(ctx *gin.Context) {
	...
	products, currentPage, _, totalItems, err := c.productUC.GetAllProducts(page, pageSize)
	if err != nil {
	...
	json.NewResponseSuccessWithPaging(ctx, products, paging, "OK")
}
...
func (c *productDelivery) CreateProduct(ctx *gin.Context) {
	...
	json.NewResponseCreated(ctx, createdProduct, "Success Create Product")
}
...
func (c *productDelivery) GetProduct(ctx *gin.Context) {
	...
	if err != nil {
		if err.Error() == "product not found" {
			json.NewResponseNotFound(ctx, "Product Not Found")
			return
		}
		json.NewResponseError(ctx, err.Error())
		return
	...
	json.NewResponseSuccessWithData(ctx, getProduct, "Success Create Product")
}
...
func (c *productDelivery) UpdateProduct(ctx *gin.Context) {
	...
	json.NewResponseCreated(ctx, result, "Success Update Product")
}
...
func (c *productDelivery) DeleteProduct(ctx *gin.Context) {
	...
	json.NewResponseSuccess(ctx, "SuccessFuly Delete Product")
}
